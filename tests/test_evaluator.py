...
def test_evaluate_clustering_quality(evaluator, sample_features, sample_labels):
    """测试聚类质量评估"""
    # 评估聚类质量
    results = evaluator.evaluate_clustering_quality(sample_features, sample_labels)
    ...
    assert "homogeneity_score" in results
    assert "v_measure_score" in results
    ...
    assert 0 <= results["homogeneity_score"] <= 1
    assert 0 <= results["v_measure_score"] <= 1
