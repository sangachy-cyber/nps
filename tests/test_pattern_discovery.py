...
def test_different_behavior_types(
    ...
    # 至少识别到4种预期的行为类型
    assert len(actual_labels.intersection(expected_labels)) >= 4, f"未识别到足够的预期行为类型，实际识别到: {actual_labels}, 预期: {expected_labels}"

    # 验证所有标签都是有效的行为类型
    # 从pattern_identifier实例获取有效标签
    valid_labels = set(pattern_identifier.BEHAVIOR_LABELS.values())
    for label in labels:
        assert label in valid_labels, f"无效的行为标签: {label}"
