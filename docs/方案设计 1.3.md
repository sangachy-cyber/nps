...
- **输入**：CSV 文件（timestamp, delay, loss_rate）
...
| `feat_delay_trend` | 长期趋势 | 🟡 | Theil-Sen 斜率（预留） |
| ❌ `feat_loss_pattern_std` | — | ❌ | 冗余，删除 |
| ❌ `feat_loss_unique_values` | — | ❌ | 作用有限，删除 |
...
| --- | --- | --- |
| `0` | Stable | 无显著异常 |
| `1` | Weak Burst | loss ∈ [0.1, 0.4) 或 ramp_up > 100ms |
| `2` | Strong Burst | 存在 ≥20 连续点满足：delay ≥ 500ms & loss ≥ 0.3 |
| `3` | Instant Spike | ≤3 点满足：loss ≥ 0.9 或 delay ≥ 1000ms |
| `4` | High Delay No Loss | delay_mean > 600ms 且 loss_mean < 0.01 |

> 采用子段扫描，避免混合窗口降级 输出确定性整数标签（0~4），无噪声类（-1）
> 
