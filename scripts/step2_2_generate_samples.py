...
def generate_samples(
    ...
    if len(unique_non_noise_labels) > 0:
        ...
        for i in noise_indices:
            ...
            for label, sequence in label_sequences.items():
                ...
                distance = dtw_distance(noise_sequence, sequence)
    ...
    for i, start_idx in enumerate(start_indices):
        ...
        #     validation_fixed = constraint_injector.validate_sequence(
        #         delay_fixed, loss_rate_fixed
        #     )
        #     print(f"修复后数据验证结果: {validation_fixed}")

        # 保存当前组的生成数据
        original_file = (
            output_generation_dir
            / f"original_sample_6000_group_{i + 1}_behavior_{group_behavior_types[i]}.csv"
        )
        generated_file = (
            output_generation_dir
            / f"generated_sample_6000_group_{i + 1}_behavior_{group_behavior_types[i]}.csv"
        )
