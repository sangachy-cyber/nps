...
def train_model(input_preprocess_dir: Path, output_train_dir: Path, max_samples: int = None):
    ...
    if max_samples is not None and len(features) > max_samples:
        print(f"样本数量过大 ({len(features)}), 将使用前 {max_samples} 个样本进行训练")
    ...
    print(f"每个批次序列长度: {batch_seq_len}")
    ...
    print(f"原始数据形状: ({num_samples}, 2)")
    print(f"批次数据形状: {features_tensor.shape}")
    ...
    for epoch in range(num_epochs):
        ...
        print(f"训练轮次 [{epoch + 1}/{num_epochs}], 平均损失值: {avg_epoch_loss:.4f}")
