...
def preprocess_data(
    ...
    logger.info(f"处理后数据总样本数: {total_samples}")
    ...
    if num_behavior_labels != total_windows:
        logger.warning(
            f"行为标签数量 {num_behavior_labels} 与窗口数量 {total_windows} 不匹配"
        )
        # 改进行为标签调整逻辑
        if num_behavior_labels > total_windows:
            # 行为标签数量过多，采用均匀采样方式保留
            ...
            logger.info(f"已均匀采样行为标签数量为 {len(behavior_ids)}")
        else:
            # 行为标签数量不足，使用插值方式填充
            # 计算需要填充的标签数量
            ...
            if padding > 0:
                ...
                logger.info(f"已使用插值方式填充行为标签数量为 {len(behavior_ids)}")
    ...
    if np.any(behavior_labels == -2):
        logger.warning(f"{np.sum(behavior_labels == -2)} 个样本没有分配行为标签")
        # 优化未分配标签的填充逻辑
        # 首先填充开头的未分配标签
        for t in range(total_samples):
    ...
    for behavior, count in zip(unique_behaviors, counts):
        logger.info(f"类别 {behavior}: {count} 个样本 ({count / total_samples * 100:.2f}%)")

    # 2. 剔除噪声标签（-1）
    non_noise_mask = behavior_labels != -1
    valid_samples = non_noise_mask.sum()
    logger.info(f"有效样本数（排除噪声）: {valid_samples}/{total_samples}")
    ...
    logger.info(f"均衡前样本分布: {label_distribution}")
    ...
    if other_labels:
        ...
        if most_common_count > 2 * avg_other_count:
            ...
            logger.info(f"均衡后样本分布: {balanced_distribution}")
            logger.info(f"已将类别 {most_common_label} 的样本数从 {most_common_count} 减少到 {target_count}")
    ...
    logger.info("Normalizing delay values...")
    ...
    if normalization_method == 'robust':
        ...
        logger.info(f"使用{normalization_method}归一化方法")
        logger.info(f"分位数参数 - 数据点数: {len(quantile_params['sorted_data'])}")
    ...
    logger.info(f"训练数据丢包率唯一值: {unique_loss_rates}")
    ...
    logger.info(f"选择的样本长度: {len(selected_df)}")
    ...
    logger.info(f"所有原始行为: {all_original_behaviors}")
    logger.info(f"唯一行为: {np.unique(selected_behavior_labels)}")
    logger.info(f"行为映射: {behavior_mapping}")
    ...
    logger.info(f"行为ID形状: {selected_behavior_ids.shape}")
    logger.info(f"行为数量: {num_behaviors}")
